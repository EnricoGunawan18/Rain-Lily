...
public class still8Script : MonoBehaviour
{
	[SerializeField]
	Button Play;
	...
	void PlayButton()
	{
		...
		PlayerPrefs.SetInt("ResetPos", 122);//////////////////change each script pls(Enrico reminder)
		PlayerPrefs.SetInt("NovelMenu", 28);//////////////////change each script pls(Enrico reminder)

		int[] startDate = { 10, 25 };//////////////////change each script pls(Enrico reminder)
		PlayerPrefsX.SetIntArray("Date", startDate);

		PlayerPrefs.SetInt("Money", 0);
		PlayerPrefs.SetFloat("LiedHeart", 0);//////////////////change each script pls(Enrico reminder)
		PlayerPrefs.SetFloat("KleinHeart", 20);//////////////////change each script pls(Enrico reminder)
